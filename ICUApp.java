...
public class ICUApp extends JFrame {
    ...
    private void showLoginScreen() {
        ...
        // ✅ FIXED: Launch FaceDetection instead of camzra
        loginButton.addActionListener(e -> {
            SwingUtilities.getWindowAncestor(loginButton).dispose(); // Close login window
            SwingUtilities.invokeLater(() -> {
                try {
                    new gui.FaceDetection().setVisible(true); // Opens Face Detection window
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(
                        ICUApp.this,
                        "Could not start camera: " + ex.getMessage(),
                        "Launch Error",
    ...
    public static class SplashScreen extends JWindow {
        ...
        public SplashScreen() {
            ...
            fadeTimer = new Timer(60, e -> {
                ...
                setOpacity(Math.min(1.0f, Math.max(0.0f, opacity)));            });
            fadeTimer.start();
            setOpacity(0f);
            setVisible(true);
        }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, "OpenCV failed to initialize.\nApplication will close.", "Critical Error", JOptionPane.ERROR_MESSAGE);
                System.exit(1);
            }
